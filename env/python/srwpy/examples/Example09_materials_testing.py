...
print(f'Simulating propagation of a Coherent Gaussian X-ray beam through a Beamline containing Pt and B4C Mirrors')
...
B4C_refl=get_refl_arr('B4C', _n_ang=_n_ang, _n_ph_en=_n_ph_en, _n_comp=_n_comp, _ph_en_start=_ph_en_start, _ph_en_fin=_ph_en_fin, _ph_en_scale_type=_ph_en_scale_type,
                  _ang_start=_ang_start, _ang_fin=_ang_fin, _ang_scale_type=_ang_scale_type)



# plot_refl3d(Pt_refl, _n_ang=_n_ang, _n_ph_en=_n_ph_en, _ph_en_start=_ph_en_start, _ph_en_fin=_ph_en_fin,
#                   _ang_start=_ang_start, _ang_fin=_ang_fin)
# plot_refl3d(B4C_refl, _n_ang=_n_ang, _n_ph_en=_n_ph_en, _ph_en_start=_ph_en_start, _ph_en_fin=_ph_en_fin,
#                   _ang_start=_ang_start, _ang_fin=_ang_fin)


#**********************Input Parameters and Structures
#***********Folder and Data File Names
strDataFolderName = 'data_NW_example' #data sub-folder name
strMirSurfHeightErrInFileName01 = 'mirror2_1d.dat'
strIntOutFileName = 'ex22_res_int_in.dat' #initial wavefront intensity distribution output file name
strPhOutFileName = 'ex22_res_phase_in.dat' #initial wavefront phase output file name
strPropOutFileName = 'ex22_res_prop.dat' #propagated wavefront intensity distribution output file name

strIntPropOutFileName = 'ex22_res_int_prop.dat' #propagated wavefront intensity distribution output file name
strPhasePropOutFileName  = 'ex22_res_phase_prop.dat' #propagated wavefront phase output file name
strIntPropOutFileName2 = 'ex22_res_int_prop_Pt.dat' #propagated wavefront intensity distribution output file name
strPhPropOutFileName2  = 'ex22_res_phase_prop_Pt.dat' #propagated wavefront phase output file name
strIntPropOutFileName3 = 'ex22_res_int_prop_B4C.dat' #propagated wavefront intensity distribution output file name
strPhPropOutFileName3  = 'ex22_res_phase_prop_B4C.dat' #propagated wavefront phase output file name
...
distHFM_Samp = 10 #Distance from HFM to Sample [m]


lenKB = 1 #Length of VFM and HFM (same for each) [m]
angKB = 2.5e-3 #Incident Angle of VFM and HFM [rad]
...
prParPost = [0, 0, 1., 3, 0, 1,    1,  1,   1.,  0, 0, 0] #Post-propagation resizing parameters
...
optBL = SRWLOptC([opApKB,   opVFM],#opDrVFM_HFM,     opHFM],#     opDrHFM_Samp],
                 [prParInit, prPar0])#      prPar0,    prPar0])
...
# #                     [prPar0,    prPar0,     prPar0,         prPar0,   prPar0,  prParPost])
optBL_Pt = SRWLOptC([opApKB, opVFM_Pt],
                 [prParInit,  prPar0])
...
#                      _ang_start=_ang_start, _ang_fin=_ang_fin, _ang_scale_type=_ang_scale_type)
# # # opTrErVFM_B4C = srwl_opt_setup_surf_height_1d(heightProfData, _dim='y', _ang=angKB, _amp_coef=1)
...
#                      _refl=B4C_refl, _n_ang=_n_ang, _n_ph_en=_n_ph_en, _n_comp=_n_comp, _ph_en_start=_ph_en_start, _ph_en_fin=_ph_en_fin, _ph_en_scale_type=_ph_en_scale_type,
#                      _ang_start=_ang_start, _ang_fin=_ang_fin, _ang_scale_type=_ang_scale_type)
# # # opTrErHFM_B4C = srwl_opt_setup_surf_height_1d(heightProfData, _dim='x', _ang=angKB, _amp_coef=1)

# # #"Beamline" - a sequenced Container of Optical Elements (together with the corresponding wavefront propagation parameters,
# # #and the "post-propagation" wavefront resizing parameters for better viewing).
# # # optBL_B4C = SRWLOptC([opVFM_B4C, opTrErVFM_B4C, opDrVFM_HFM, opHFM_B4C, opTrErHFM_B4C, opDrHFM_Samp],
# # #                     [prPar0,    prPar0,     prPar0,         prPar0,   prPar0,  prParPost])
# optBL_B4C = SRWLOptC([opApKB, opVFM_B4C],# opDrVFM_HFM, opHFM_B4C],#opDrHFM_Samp],
#                  [prParInit,  prPar0])#      prPar0,    prPar0])#      prParPost])





plotMesh0x = [1e+06*mesh0.xStart, 1e+06*mesh0.xFin, mesh0.nx]
plotMesh0y = [1e+06*mesh0.yStart, 1e+06*mesh0.yFin, mesh0.ny]
...
print('   Propagating Wavefront (through two eliptical mirrors)... ', end='')
...
# srwl.CalcIntFromElecField(arI3, wfr3, 6, 0, 3, mesh3.eStart, 0, 0) #extracts intensity
...
# srwl.CalcIntFromElecField(arI3x, wfr3, 6, 0, 1, mesh3.eStart, 0, 0) #extracts intensity
# arI3y = array('f', [0]*mesh3.ny) #array to take 1D intensity data
# srwl.CalcIntFromElecField(arI3y, wfr3, 6, 0, 2, mesh3.eStart, 0, 0) #extracts intensity
# arP3 = array('d', [0]*mesh3.nx*mesh3.ny) #"flat" array to take 2D phase data (note it should be 'd')
# srwl.CalcIntFromElecField(arP3, wfr3, 0, 4, 3, mesh3.eStart, 0, 0) #extracts radiation phase
...
uti_plot2d1d(arI1, plotMesh1x, plotMesh1y, labels=['Horizontal Position [microns]', 'Vertical Position [microns]', 'Intensity After Propagation through 2 Eliptical Mirrors'])

plotMesh2x = [1e+06*mesh2.xStart, 1e+06*mesh2.xFin, mesh2.nx]
plotMesh2y = [1e+06*mesh2.yStart, 1e+06*mesh2.yFin, mesh2.ny]
uti_plot2d1d(arI2, plotMesh2x, plotMesh2y, labels=['Horizontal Position [microns]', 'Vertical Position [microns]', 'Intensity After Propagation through 2 Platinum Eliptical Mirrors'])

# plotMesh3x = [1e+06*mesh3.xStart, 1e+06*mesh3.xFin, mesh3.nx]
# plotMesh3y = [1e+06*mesh3.yStart, 1e+06*mesh3.yFin, mesh3.ny]
# uti_plot2d1d(arI3, plotMesh3x, plotMesh3y, labels=['Horizontal Position [microns]', 'Vertical Position [microns]', 'Intensity After Propagation through 2 Boron Carbide Eliptical Mirrors'])



#calculate relative difference between the two propagated wavefronts and predicted reflectivity
print()
...
print("relative reflectivity between Pt and perfect mirrors: ", (max(arI2))/max(arI1))
# print("relative reflectivity between B4C and perfect mirrors: ", (max(arI3))/max(arI1))

print("predicted reflectivity for Pt mirror (s): ", mirror_reflectivity('Pt', 0.003, 12402))
print("predicted reflectivity for Pt mirror (p): ", mirror_reflectivity('Pt', 0.003, 12402, polarization='p'))
# print("predicted reflectivity for B4C mirror: ", mirror_reflectivity('B4C', angKB, avgEn))


# get_indices_from_arr(64000, _n_ang=_n_ang, _n_ph_en=_n_ph_en, _ph_en_start=_ph_en_start, _ph_en_fin=_ph_en_fin, _ang_start=_ang_start, _ang_fin=_ang_fin, _n_comp=_n_comp)
